document.onkeydown = function (e) {
    ...
    if (current.type === 'textarea' || current.type === 'text' || current.type === 'search') {
        ...
        var imgUrl = 'images/' + prefix + '_' + rand(1, 4) + '.svg';
        if($('#checkkata').prop('checked')==true) {
...
document.body.addEventListener( "click", function( event ) {
    ...
    var y = event.pageY ;
    var size = rand(40, 50);
    var imgUrl = 'images/' + 'pochi' + '_' + rand(1, 4) + '.svg';
    
    if($('#checkpochi').prop('checked')==true) {
        outputimage(x,y,imgUrl,size);
    }
} ) ;
...
document.body.addEventListener("mousemove", function(event){
    ...
    var size = rand(60, 70);
    var imgUrl = 'images/' + 'byun' + '_' + rand(1, 4) + '.svg';
    if($('#checkbyun').prop('checked')==true) {
        outputimage(x,y,imgUrl,size,isEnter=false,mousemove=true,lastx=lastx,lasty=lasty);
    }
    lastx = x;
    lasty = y;
